...
export async function AddBuyingHistory(req, res) {
  try {
    const history = await req.body;
    history.timeStamp = Date.now();
    history.paymentDate = new Date().toLocaleDateString();
    const client = await DBConnection();
    const savePaymentHistory = await client
      .db("PaymentHistory")
      .collection("histories")
      .insertOne(history);
    if (savePaymentHistory?.insertedId) {
      return res.status(200).json({
...
export async function GetBuyingHistory(req, res) {
  try {
    const client = await DBConnection();
    const historis = await client
      .db("PaymentHistory")
      .collection("histories")
      .find()
      .sort({ timeStamp: -1 })
...
export async function DeleteReviews(req, res) {
  try {
    const { propertyId, rewiewId } = await req.body;
    const propertyFilter = {
      _id: new ObjectId(propertyId),
    };
    const reviewFilter = {
      _id: new ObjectId(rewiewId),
    };
    const client = await DBConnection();
    ...
    const userReviews = await client
      .db("Agent_propertises")
      .collection("propertise")
      .updateOne(propertyFilter, {
        $pull: {
          reviews: reviewFilter,
        },
      });
    if (userReviews?.modifiedCount > 0) {
      return res.status(200).json({
